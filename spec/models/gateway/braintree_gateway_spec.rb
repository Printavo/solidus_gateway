...
describe Spree::Gateway::BraintreeGateway do
  let(:gateway) { described_class.create!(name: "Braintree Gateway", active: true) }
  ...
  let(:credit_card) { create(:credit_card, verification_value: "123", number: credit_card_number, month: Time.current.month, year: Time.current.year + 1, name: "John Doe", cc_type: "mastercard") }
  ...
  before do
    ...
    with_payment_profiles_off do
      ...
      create(:payment, source: credit_card, order: order, payment_method: gateway, amount: 10.00)
    end
  end
  ...
  describe "payment profile creation" do
    ...
    context "when a credit card is created" do
      it "saves the address associated on the remote payment profile" do
  ...
  describe "payment profile failure" do
    let(:credit_card_number) { "5105105105105100" }
  ...
  describe "authorize" do
    context "when the credit card has a token" do
      before do
    ...
    context "when the given credit card does not have a token" do
      context "and the credit card has a customer profile id" do
        before do
      ...
      context "but no customer profile id" do
    ...
    it "returns a success response with an authorization code" do
    ...
    shared_examples "a valid credit card" do
      it "works through the spree payment interface" do
        ...
        expect(transaction.credit_card_details.expiration_date).to eq "#{Time.current.month}/#{Time.current.year + 1}"
  ...
  describe "update_card_number" do
    it "passes through gateway_payment_profile_id" do
      credit_card_attrs = { "token" => "testing", "last_4" => "1234", "masked_number" => "555555******4444" }
      gateway.update_card_number(payment.source, credit_card_attrs)
  ...
  def credit_using_spree_interface
    ...
    expect(transaction.credit_card_details.expiration_date).to eq "#{Time.current.month}/#{Time.now.year + 1}"
    expect(transaction.customer_details.first_name).to eq "John"
    expect(transaction.customer_details.last_name).to eq "Doe"
  end
